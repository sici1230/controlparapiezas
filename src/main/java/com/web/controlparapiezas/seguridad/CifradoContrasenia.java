...
public class CifradoContrasenia {
    ...
    private static CifradoContrasenia instancia;
    
    private VariablesDeEntorno variables = new VariablesDeEntorno();

    public CifradoContrasenia() {
    }

    public String cifrar(String contrasenia) {
        try {
            System.out.println("contraseña " + contrasenia);
            byte[] salt = variables.getSALT().getBytes();
            int iterationCount = variables.getITERATION_COUNT();
            int keyLength = variables.getKEY_LENGTH();
            SecretKeySpec key = createSecretKey(contrasenia.toCharArray(), salt, iterationCount, keyLength);

            String originalPassword = contrasenia;
